...
- Version announcement (**currently commented out** during rapid dev): on boot, if `package.json`'s `version` differs from the last-announced one (in `meta`), posts `🚀 ¡Nueva versión X.Y.Z! …` into General — once per version, not per restart. Re-enable the `announceVersion()` call in `startBot()` and bump `version` before a deploy to trigger it
